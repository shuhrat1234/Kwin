...
def cart_add(request, add_id):
    if not request.user.is_authenticated:
        messages.error(request, get_error_message("not_authenticated", request) or "Пожалуйста, войдите для добавления в корзину!")
        print(request.user.is_authenticated)
        return redirect(request.META.get("HTTP_REFERER", "products"))

    product = Product.objects.filter(id=add_id).first()
    if not product:
        messages.error(request, get_error_message("product_not_found", request) or "Товар не найден!")
        return redirect(request.META.get("HTTP_REFERER", "products"))
    ...
    if not created:
        ...
        cart_item.save()
        messages.error(request, get_error_message("already_in_cart", request) or "Товар уже в корзине, количество обновлено!")
        return redirect(request.META.get("HTTP_REFERER", "products"))

    messages.error(request, get_error_message("cart_added", request) or "Товар успешно добавлен в корзину!")
    return redirect(request.META.get("HTTP_REFERER", "products"))
@login_required
